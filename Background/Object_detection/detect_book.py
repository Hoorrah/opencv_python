...
def getContours(img):
	...
	imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # convert the image from color space BGR to gray space
	...
	for cnt in contours:#go through all contours
		...
		if area > 1000: #if the contour area is small skip it and just draw contours that their area is more than 1000
			return cv2.drawContours(img, cnt, -1, (0,0,0), 5) #1. source image, 2. the contours which should be passed as a Python list, 3. index of contours To draw all contours, pass -1. 4.color,5. thickness
...
#function for crpping image background vertically
def crop_vertically(img, imgContour, num_image, patch_size_v, limit): #1.orginal image , 2.image with its contours 3.number of image 4.patch szie 5.limit for mean squar error of color channeles
	#define variables
...
input_path = r'C:\Users\Windows10\Desktop\Internship\Image\Book-image-2' #path to our data
...
for f in os.listdir(input_path):# go through folder path and read images one by one
	...
	if (if_background(img , type = 1)): #1.source image 2. type of cropping is vertically from right of image
		...
		img_v = crop_vertically(img, imgContour, num_image, patch_size_v=10, limit=300)#1.orginal image , 2.image with its contours 3.number of image 4.patch szie 5.limit for mean squar error of color channeles
	else: #there is no background
		img_v = crop_vertically(img, img, num_image, patch_size_v=20, limit=350)#1.orginal image , 2.image with its contours 3.number of image 4.patch szie 5.limit for mean squar error of color channeles
