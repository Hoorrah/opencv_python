...
def if_backgrnd_hor(img):
		...
		if ( mse(prev_B , B) <= 350 and mse(prev_G , G) <= 350 and mse(prev_R , R) <= 350):
			check_hor = False
...
input_path = r'C:\Users\Windows10\Desktop\Internship\Image\chera'
...
for f in os.listdir(input_path):
	...
	if not (if_backgrnd_hor(img)):
		...
		crop_horizontally(img, imgContour, num_image, 10, 600)
		print(num_image)
	else:
		crop_horizontally(img, img, num_image, 20, 350)
